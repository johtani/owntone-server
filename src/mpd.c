...
struct mpd_command
{
   ...
   * @return MPD ack error (ACK_ERROR_NONE on succes)
   */
  enum mpd_ack_error (*handler)(struct evbuffer *evbuf, int argc, char **argv, char **errmsg, struct mpd_client_ctx *ctx);
  int min_argc;
};
...
/*
 * Parses a range argument of the form START:END (the END item is not included in the range)
...
static int
notify_idle_client(struct mpd_client_ctx *client_ctx, short events)
{
...
mpd_command_currentsong(struct evbuffer *evbuf, int argc, char **argv, char **errmsg, struct mpd_client_ctx *ctx)
{
  ...
  if (ret < 0)
    {
      *errmsg = safe_asprintf("Error setting media info for file with id: %d", status.id);
...
mpd_command_idle(struct evbuffer *evbuf, int argc, char **argv, char **errmsg, struct mpd_client_ctx *ctx)
{
  ...
  if (ctx->events & ctx->idle_events)
    notify_idle_client(ctx, ctx->events);
...
mpd_command_pause(struct evbuffer *evbuf, int argc, char **argv, char **errmsg, struct mpd_client_ctx *ctx)
{
  ...
  if (ret < 0)
    {
      *errmsg = safe_asprintf("Failed to %s playback", (pause == 1) ? "pause" : "start");
      return ACK_ERROR_UNKNOWN;
    }
...
mpd_command_move(struct evbuffer *evbuf, int argc, char **argv, char **errmsg, struct mpd_client_ctx *ctx)
{
  ...
  if (!(start_pos >= 0 && start_pos < queue_length
    ...
    {
      *errmsg = safe_asprintf("Range too large for target position %d or bad song index (count %d, length %u)", to_pos, count, queue_length);
      return ACK_ERROR_ARG;
    }
...
mpd_command_plchanges(struct evbuffer *evbuf, int argc, char **argv, char **errmsg, struct mpd_client_ctx *ctx)
{
  ...
  ack_error = plchanges_build_queryparams(&query_params, argc, argv, errmsg);
...
mpd_add_directory(struct evbuffer *evbuf, int directory_id, int listall, int listinfo, char **errmsg)
{
  ...
  if (ret < 0)
    {
      *errmsg = safe_asprintf("Failed to start directory enum for parent_id %d\n", directory_id);
...
mpd_command_listall(struct evbuffer *evbuf, int argc, char **argv, char **errmsg, struct mpd_client_ctx *ctx)
{
  ...
  return mpd_add_directory(evbuf, dir_id, 1, 0, errmsg);
}
...
mpd_command_listallinfo(struct evbuffer *evbuf, int argc, char **argv, char **errmsg, struct mpd_client_ctx *ctx)
{
  ...
  return mpd_add_directory(evbuf, dir_id, 1, 1, errmsg);
}
...
mpd_command_lsinfo(struct evbuffer *evbuf, int argc, char **argv, char **errmsg, struct mpd_client_ctx *ctx)
{
  ...
  ack_error = mpd_add_directory(evbuf, dir_id, 0, 1, errmsg);

  // If the root directory was passed as argument add the stored playlists to the response
  if (ack_error == ACK_ERROR_NONE && print_playlists)
    {
...
struct mpd_sticker_command {
  const char *cmd;
  enum mpd_ack_error (*handler)(struct evbuffer *evbuf, int argc, char **argv, char **errmsg, const char *virtual_path);
  int need_args;
};
...
mpd_command_sticker(struct evbuffer *evbuf, int argc, char **argv, char **errmsg, struct mpd_client_ctx *ctx)
{
  ...
  ack_error = cmd_param->handler(evbuf, argc, argv, errmsg, virtual_path);
...
static enum mpd_ack_error
mpd_command_close(struct evbuffer *evbuf, int argc, char **argv, char **errmsg, struct mpd_client_ctx *ctx)
{
...
mpd_command_outputvolume(struct evbuffer *evbuf, int argc, char **argv, char **errmsg, struct mpd_client_ctx *ctx)
{
  ...
  return outputvolume_set(shortid, volume, errmsg);
}
...
static enum mpd_ack_error
mpd_command_command_list_begin(struct evbuffer *evbuf, int argc, char **argv, char **errmsg, struct mpd_client_ctx *ctx)
{
...
static enum mpd_ack_error
mpd_command_command_list_ok_begin(struct evbuffer *evbuf, int argc, char **argv, char **errmsg, struct mpd_client_ctx *ctx)
{
...
static enum mpd_ack_error
mpd_command_command_list_end(struct evbuffer *evbuf, int argc, char **argv, char **errmsg, struct mpd_client_ctx *ctx)
{
  if (ctx->cmd_list_type == COMMAND_LIST_BEGIN)
    ...
    {
      *errmsg = safe_asprintf("Got with command list end without preceeding list start");
      return ACK_ERROR_ARG;
    }
...
// Parses the argument string into an array of strings. Arguments are separated
// by a whitespace character and may be wrapped in double quotes. ack_error is
// set and errmsg allocated if there is an error.
static enum mpd_ack_error
mpd_parse_args(char **argv, int argv_size, int *argc, char *args, bool *must_disconnect, char **errmsg)
{
  char *input = args;
  int arg_count = 0;
  ...
  if (arg_count == 0)
    {
      ...
      *must_disconnect = true; // in this case MPD disconnects the client
      return ACK_ERROR_ARG;
    }
  ...
  if (*input != 0 && arg_count == argv_size)
    {
      *errmsg = safe_asprintf("Too many arguments: %d allowed", argv_size);
      return ACK_ERROR_ARG; // in this case MPD doesn't disconnect the client
    }
...
static enum mpd_ack_error
mpd_list_add(struct mpd_client_ctx *client_ctx, const char *line)
{
  size_t sz = strlen(line) + 1;
  int ret;

  if (evbuffer_get_length(client_ctx->cmd_list_buffer) + sz > MPD_MAX_COMMAND_LIST_SIZE)
    {
  ...
  ret = evbuffer_add(client_ctx->cmd_list_buffer, line, sz);
  if (ret < 0)
    {
      DPRINTF(E_LOG, L_MPD, "Failed to add to command list\n");
      client_ctx->must_disconnect = true;
      return ACK_ERROR_NONE;
    }
...
static enum mpd_ack_error
mpd_process_command_line(struct evbuffer *output, char *line, int cmd_num, struct mpd_client_ctx *client_ctx)
{
  enum mpd_ack_error ack_error;
  bool got_noidle;
  char *argv[MPD_COMMAND_ARGV_MAX] = { 0 }; // Zero init just to silence false positive from scan-build
  int argc = 0; // Also to silence scan-build
  char *cmd_name = NULL;
  ...
  if ((client_ctx->cmd_list_type == COMMAND_LIST_BEGIN || client_ctx->cmd_list_type == COMMAND_LIST_OK_BEGIN) && strcmp(line, "command_list_end") != 0)
    {
      return mpd_list_add(client_ctx, line);
    }

  got_noidle = (strcmp(line, "noidle") == 0);
  if (got_noidle && !client_ctx->is_idle)
    {
      return ACK_ERROR_NONE; // Just ignore, don't proceed to send an OK
    }
  else if (!got_noidle && client_ctx->is_idle)
    {
      errmsg = safe_asprintf("Only 'noidle' is allowed during idle");
      ack_error = ACK_ERROR_ARG;
      client_ctx->must_disconnect = true;
      goto error;
    }

  // Split the read line into command name and arguments
  ack_error = mpd_parse_args(argv, MPD_COMMAND_ARGV_MAX, &argc, line, &client_ctx->must_disconnect, &errmsg);
  if (ack_error != ACK_ERROR_NONE)
    {
      goto error;
    }

  CHECK_NULL(L_MPD, argv[0]);

  cmd_name = argv[0];
  if (strcmp(cmd_name, "password") != 0 && !client_ctx->authenticated)
    {
      errmsg = safe_asprintf("Not authenticated");
      ack_error = ACK_ERROR_PERMISSION;
      goto error;
    }
  ...
  if (!command)
    {
      errmsg = safe_asprintf("Unknown command");
      ack_error = ACK_ERROR_UNKNOWN;
      goto error;
    }
  else if (command->min_argc > argc)
    {
      errmsg = safe_asprintf("Missing argument(s), expected %d, given %d", command->min_argc, argc);
      ack_error = ACK_ERROR_ARG;
      goto error;
    }

  ack_error = command->handler(output, argc, argv, &errmsg, client_ctx);
  if (ack_error != ACK_ERROR_NONE)
    {
      goto error;
    }
...
static void
mpd_process_command_list(struct evbuffer *output, struct mpd_client_ctx *client_ctx)
{
  char *line;
  ...
  while ((line = evbuffer_readln(client_ctx->cmd_list_buffer, NULL, EVBUFFER_EOL_NUL)))
    {
      ack_error = mpd_process_command_line(output, line, cmd_num, client_ctx);
      cmd_num++;
    ...
    void *ctx)
{
   ...
   */
  evbuffer_add(client_ctx->evbuffer, "OK MPD 0.20.0\n", 14);
  ...
  while (client)
    {
      ...
      notify_idle_client(client, event_mask);

      evbuffer_add_printf(client->evbuffer, "OK\n");
